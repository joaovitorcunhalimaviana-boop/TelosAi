...
function StatusBadge({ status }: { status: TodayTask['status'] }) {
    switch (status) {
        ...
        case 'in_progress':
            return (
                <span className="inline-flex items-center gap-1 text-[10px] md:text-xs font-medium px-1.5 md:px-2 py-0.5 rounded-full" style={{ color: '#D4AF37', backgroundColor: 'rgba(212, 175, 55, 0.15)', border: '1px solid rgba(212, 175, 55, 0.3)' }}>
        ...
        case 'pending_today':
            return (
                <span className="inline-flex items-center gap-1 text-[10px] md:text-xs font-medium px-1.5 md:px-2 py-0.5 rounded-full" style={{ color: '#14BDAE', backgroundColor: 'rgba(13, 115, 119, 0.15)', border: '1px solid rgba(13, 115, 119, 0.3)' }}>
            ...
            <div className="group relative flex gap-3 md:gap-4 p-3 md:p-4 rounded-lg md:rounded-xl glass-card border-none transition-all duration-300 overflow-hidden" style={{ backgroundColor: '#111520' }}>
                ...
                <Link href={`/paciente/${task.patientId}/editar`} className="flex gap-3 md:gap-4 flex-1 min-w-0 cursor-pointer">
                    <div className="flex-shrink-0">
                        <div className="h-8 w-8 md:h-10 md:w-10 rounded-full flex items-center justify-center" style={iconBgStyle}>
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start gap-2 mb-1">
                            <div className="min-w-0">
                                <h4 className="font-semibold text-xs md:text-sm transition-colors truncate" style={{ color: '#F0EAD6' }}>
                                    {task.patientName}
                                </h4>
        ...
        <Card style={{ backgroundColor: '#161B27', borderColor: '#1E2535' }}>
            <CardHeader className="flex flex-row items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                    <Badge variant="outline" className="p-1.5 md:p-2 rounded-lg shrink-0" style={{ backgroundColor: 'rgba(13, 115, 119, 0.15)', color: '#14BDAE', borderColor: '#0D7377' }}>
                        <ClipboardList className="h-4 w-4 md:h-5 md:w-5" />
                    </Badge>
                    <div className="min-w-0">
                        <CardTitle className="text-base md:text-xl font-bold flex items-center gap-2" style={{ color: '#F0EAD6' }}>
                            <span className="truncate">Tarefas de Hoje</span>
                            {totalTasks > 0 && (
                                <span className="text-[10px] md:text-xs font-medium px-1.5 md:px-2 py-0.5 rounded-full shrink-0" style={{ backgroundColor: '#0D7377', color: '#F0EAD6' }}>
                                    {totalTasks}
                                </span>
                            )}
                        </CardTitle>
                        <p className="text-[10px] md:text-xs mt-0.5 md:mt-1 hidden sm:block" style={{ color: '#7A8299' }}>
            ...
            <CardContent>
                {loading && totalTasks === 0 ? (
                    <div className="space-y-4">
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="flex gap-4 p-4 rounded-xl shadow-sm animate-pulse" style={{ backgroundColor: '#111520', border: '1px solid #1E2535' }}>
                                <div className="h-10 w-10 rounded-full" style={{ backgroundColor: '#2A3147' }} />
                                <div className="flex-1 space-y-2">
                                    <div className="h-4 w-1/3 rounded" style={{ backgroundColor: '#2A3147' }} />
                                    <div className="h-3 w-3/4 rounded" style={{ backgroundColor: '#1E2535' }} />
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="text-center py-10">
                        <CheckCircle2 className="h-10 w-10 mx-auto mb-3 opacity-60" style={{ color: '#1A8C6A' }} />
                        <p className="text-sm font-medium" style={{ color: '#D8DEEB' }}>Nenhuma tarefa pendente hoje!</p>
                        <p className="text-xs mt-1 opacity-70" style={{ color: '#7A8299' }}>Todos os follow-ups foram respondidos</p>
                    </div>
                ) : (
                    <motion.div
                        variants={container}
                    ...
                    >
                        ...
                        {tasks.overdue.length > 0 && (
                            <>
                                <SectionHeader title="Atrasados" count={tasks.overdue.length} color="text-red-500" />
                        ...
                        {tasks.inProgress.length > 0 && (
                            <>
                                <SectionHeader title="Em andamento" count={tasks.inProgress.length} color="text-amber-400" />
                        ...
                        {tasks.pendingToday.length > 0 && (
                            <>
                                <SectionHeader title="Pendentes hoje" count={tasks.pendingToday.length} color="text-teal-400" />
